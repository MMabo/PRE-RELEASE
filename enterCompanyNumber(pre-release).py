  ...
  def Run(self):
    ...
    while Choice != "Q":
      ...
      if Choice == "1":
        ...
        while Index == -1:
          NumberedCompanies = []
          for C in enumerate(self._Companies):
              print(C[0]+1,C[1].GetName())
          CompanyNumber = 0
          while CompanyNumber <= 0 or CompanyNumber > 3:
              try:
                  CompanyNumber = int(input("Enter the number next to the company you wish to modify: "))
              except:
                  print("Invalid company number, try again.")
                  print()
          Index = CompanyNumber
