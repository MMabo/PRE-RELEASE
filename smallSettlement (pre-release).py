...
class LargeSettlement(Settlement): #INHERITANCE - inherits everything from Settlement
  def __init__(self, ExtraXSize, ExtraYSize, ExtraHouseholds):
    super(LargeSettlement, self).__init__() #INHERITANCE - inherits the _init_ of Settlement
...
class SmallSettlement(Settlement): #INHERITANCE - inherits everything from Settlement
  def __init__(self, ExtraXSize, ExtraYSize, ExtraHouseholds):
    super(SmallSettlement, self).__init__() #INHERITANCE - inherits the _init_ of Settlement
    self._XSize -= ExtraXSize
    self._YSize -= ExtraYSize
    self._StartNoOfHouseholds -= ExtraHouseholds
    for Count in range (1, self._StartNoOfHouseholds + 1):
      del self.HouseHolds[Count]
...
class Simulation:
  def __init__(self):
    ...
    if Choice == "L":
      ...
      ExtraX = int(input("Enter amount to subtract from X size of settlement: "))
      while ExtraX < 0 or 1000 - ExtraX < 1:
        ExtraX = int(input("Please enter again an amount to subtract from X size of settlement: "))
        
        ExtraY = int(input("Enter amount to subtract from Y size of settlement: "))
      while ExtraY < 0 or 1000 - ExtraX < 1:
        ExtraY = int(input("Please enter again an amount to subtract from Y size of settlement: "))
        
      ExtraHouseholds = int(input("Enter number of households to subtract from settlement: "))
      while ExtraHouseholds < 0 or 250 - ExtraHouseholds < 1:
        ExtraHouseholds = int(input("Please enter again a number of households to subtract from settlement: "))
        
      self._SimulationSettlement = SmallSettlement(ExtraX, ExtraY, ExtraHouseholds)
    else:
