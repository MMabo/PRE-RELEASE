...
class Company:
  ...
  def ProcessDayEnd(self):
    ...
    Details += "Daily costs for company: " + str(InPounds(self._DailyCosts)) + "\nCost for delivering produce to outlets: " + str(InPounds(DeliveryCosts)) + "\n"
    for Current in range (0, len(self._Outlets)):
      ...
      Details += "Outlet " + str(Current + 1) + " profit/loss: " + str(InPounds(ProfitLossFromThisOutlet)) + "\n"
      ProfitLossFromOutlets += ProfitLossFromThisOutlet
    Details += "Previous balance for company: " + str(InPounds(self._Balance)) + "\n"
    self._Balance += ProfitLossFromOutlets - self._DailyCosts - DeliveryCosts
    Details += "New balance for company: " + str(InPounds(self._Balance))
    return Details
